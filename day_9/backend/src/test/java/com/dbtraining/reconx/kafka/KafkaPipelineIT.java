 ...
 *                        the full Spring context points at the container
 *                        broker.
 *                        Awaitility polls auditRepo.count() every 500ms instead
 *                        of Thread.sleep.
 *                        WHY: A mocked Kafka client tests your code, not your
 *                        configuration.
 *                        Topic name typos, wrong partition counts, missing
 *                        groupId — all
 *                        invisible to a mock. This test catches the bugs that
 *                        only appear
 *                        when bytes actually cross a network.
 *                        OBSERVE: ./mvnw -pl backend verify
 *                        -Dtest=KafkaPipelineIT
 *                        → "Tests run: 1, Failures: 0, Errors: 0" with
 *                        Testcontainers log
 *                        confirming the cp-kafka:7.6.0 image started on a
 *                        random port.
 *
 *                        GOTCHA: Assert on (before + 100), NOT on count() ==
 *                        100 directly.
 *                        The test database may not be empty at startup (seed
 *                        data, prior tests).
 *                        Asserting on the delta protects you from false
 *                        failures.
 *                        ============================================================================
 */
@SpringBootTest
